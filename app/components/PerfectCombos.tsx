...
function Estrellas({ valoracion }: { valoracion: number }) {
  return (
    <span
      className="inline-flex items-center gap-1 text-xs font-bold text-[#2C1810]"
      aria-label={`Valoración ${valoracion} sobre 5`}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        className="size-4 text-[#FFC107]"
        fill="currentColor"
        aria-hidden="true"
      >
...
function PuntoSabor({ nombre, color }: { nombre: string; color: string }) {
  return (
    <li className="flex items-center gap-2 text-sm text-[#2C1810]">
      <span
    ...
    <>
      ...
      >
        ...
        {combo.etiquetas.length > 0 && (
          <span className="absolute bottom-3 left-3 rounded-full bg-[#E91E8C] px-3 py-1 text-xs font-extrabold text-white shadow">
            {combo.etiquetas[0]}
          </span>
        )}
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain p-5 pb-2 sm:p-6 sm:pb-2">
        <DialogHeader>
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold tracking-wide text-[#E91E8C] uppercase">
            <span>{combo.categoria}</span>
            <span className="text-[#2C1810]/30">•</span>
            <span className="text-[#2C1810]/60">{combo.formato}</span>
          </div>
          <DialogTitle className="text-xl text-[#2C1810] sm:text-2xl">
            {combo.nombre}
          </DialogTitle>
          <DialogDescription className="text-[#2C1810]/70">
            {combo.descripcion}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-baseline gap-2">
            ...
            {combo.precioAnterior && (
              <span className="text-sm text-[#2C1810]/40 line-through">
                {combo.precioAnterior}
              </span>
            )}
          </div>
          ...
          {combo.calorias > 0 && (
            <span className="rounded-full bg-[#FFF0E8] px-2.5 py-1 text-xs font-bold text-[#2C1810]/70">
              {combo.calorias} kcal
            </span>
          )}
        </div>

        <p className="text-sm leading-relaxed text-[#2C1810]/85">
          {combo.descripcionDetallada}
        </p>

        <div>
          <h3 className="mb-2 text-xs font-extrabold tracking-wide text-[#2C1810]/60 uppercase">
            Sabores
          </h3>
        ...
        <div>
          ...
          <ul className="grid gap-1.5">
            {combo.incluye.map((item) => (
              <li
                key={item}
                className="flex items-start gap-2 text-sm text-[#2C1810]/80"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
        ...
        {combo.alergenos.length > 0 && (
          <p className="rounded-2xl bg-[#FFF0E8] px-4 py-3 text-xs text-[#2C1810]/70">
      ...
      <div className="shrink-0 border-t-2 border-[#FFE0D2] bg-[#FFF7F1] px-5 py-4 sm:px-6 [padding-bottom:max(1rem,env(safe-area-inset-bottom))]">
...
export default function PerfectCombos() {
  ...
  return (
    <section
      id="combos"
      className="bg-[#FFF0E8] scroll-mt-16 px-4 py-10 sm:px-6"
    >
      <div className="container mx-auto">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold text-[#2C1810] sm:text-2xl">
              {seccion.titulo}
            </h2>
            <p className="mt-1 max-w-xl text-sm text-[#2C1810]/60">
              {seccion.subtitulo}
            </p>
          </div>
          <span className="text-xl text-amber-400" aria-hidden="true">
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2 p-3">
                    <div className="min-w-0">
                      <p className="text-xs font-semibold leading-tight text-[#2C1810] sm:text-sm">
                        {combo.nombre}
                      </p>
                      <p className="mt-0.5 text-base font-extrabold text-[#2C1810] sm:text-lg">
                        {combo.precio}
                      </p>
                    </div>
                    ...
                    <DialogTrigger asChild>
                      <button
                        ...
                        className="flex size-8 shrink-0 items-center justify-center rounded-full bg-[#E91E8C] text-white shadow transition-colors hover:bg-[#c2177a]"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="size-4"
